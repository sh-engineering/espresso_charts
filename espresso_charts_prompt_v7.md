...
> `suptitle_y` must be 0.65 or lower (reel safe zone). If `txt_suptitle` runs to 3+ lines, reduce `suptitle_size` to 36 and `subtitle_size` to 16. Adjust `duration` (2.5-4.0) and `hold_duration` (1.5-2.0) per story complexity.
